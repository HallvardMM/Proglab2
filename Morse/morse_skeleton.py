...
class morsecoder():
    ...
    def process_signal(self, signal):
        if (signal == 1 or signal == 2): # Signalet fra arduino 1 gir dot 2 gir dash
    ...
    def update_current_symbol(self, signal):
        self.current_symbol += str(signal - 1) # trekker fra 1 for at signal skal matche 0 og 1 i dictonari
        #print("CS:" + self.current_symbol)
        if(len(self.current_symbol) > 5):
            self.current_symbol=''

    def handle_symbol_end(self):
        if self.current_symbol and (self.current_symbol in self._morse_codes): #Sjekker om den har symbol og at det er i dictionarien.
            morse_symbol = self._morse_codes[self.current_symbol]
            #print("MS: "+ morse_symbol)
            self.update_current_word(morse_symbol)


    def update_current_word(self, symbol):
        self.current_word += symbol
        print("CW: " + self.current_word) # Printer ut bokstaver for å sjekke om det blir rett underveis.
        self.current_symbol = ''
